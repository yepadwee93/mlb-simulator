...
N_SIMS_SINGLE = 2_000  # 25 math models per sim = richer than 50K simple sims
N_SIMS_ALL = 1_000  # per game in simulate-all
...
@login_required
def clear_my_picks():
    from data.my_picks import clear_all_picks

    deleted = clear_all_picks(user_id=_uid())
...
def api_today_games():
    ...
    try:
        td = _today_est().isoformat()
        games = get_today_schedule(game_date=td)
        out = []
        for g in games:
            out.append(
                {
