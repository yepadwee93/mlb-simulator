...
for path in templates:
    ...
    # 1. Inject CSS into last </style> before </head>
    if 'nav-dropdown' not in src:
        # Find last </style> before </head>
        idx = src.rfind('</style>')
        if idx != -1:
            src = src[:idx] + DROPDOWN_CSS + '\n' + src[idx:]

    # 2. Replace plain bets link with dropdown if not already done
    if 'nav-dropdown' in src and 'My Bets &#9660;' not in src:
        src = re.sub(r'<a href="/bets"[^>]*>[^<]*My Bets[^<]*</a>', DROPDOWN_HTML, src)

    # 3. Remove now-redundant standalone nav links
    for pattern in [
