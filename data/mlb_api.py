...
def get_ballpark_weather(venue_name, game_time_utc: str = None):
    ...
    if game_time_utc:
        try:
            ...
            target_hour_str = gt.replace(minute=0, second=0, microsecond=0).strftime(
                "%Y-%m-%dT%H:00"
            )
            is_forecast = True
        except Exception:
    ...
    try:
        ...
        if target_hour_str and "hourly" in data:
            ...
            if idx is None:
                ...
                idx = 0
            temp_f = (
                hourly["temperature_2m"][idx] if idx < len(hourly.get("temperature_2m", [])) else 70
            )
            wind_mph = (
                hourly["wind_speed_10m"][idx] if idx < len(hourly.get("wind_speed_10m", [])) else 0
            )
            wind_deg = (
            ...
            wmo_code = (
                hourly["weather_code"][idx] if idx < len(hourly.get("weather_code", [])) else 0
            )
            precip_pct = (
                hourly.get("precipitation_probability", [0] * 100)[idx]
...
def get_savant_stats_all(year=None):
    ...
    try:
        ...
        resp = _req.get(url, timeout=15, headers={"User-Agent": "Mozilla/5.0 (compatible)"})
        resp.raise_for_status()
                ...
                for game in day.get("games", []):
                    ...
                    try:
                        ...
                        for pid in pitchers[1:]:  # all others = bullpen
                            p_stats = (
                                players.get(f"ID{pid}", {}).get("stats", {}).get("pitching", {})
                            )
...
def check_lineup_il_overlap(lineup_players: list, il_players: list) -> list:
    ...
    return [
        p
        for p in il_players
        if p.get("id") in il_ids and any(b.get("id") == p["id"] for b in lineup_players)
    ]
